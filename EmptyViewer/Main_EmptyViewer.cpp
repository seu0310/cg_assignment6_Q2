...
Vec3 compute_vertex_color(Vec3 position, Vec3 normal)
{
...
void draw_triangle(Vec3 v0, Vec3 v1, Vec3 v2, Vec3 c0, Vec3 c1, Vec3 c2)
{
    ...
    for (int y = minY; y <= maxY; ++y)
    {
        for (int x = minX; x <= maxX; ++x)
        {
            ...
            if (inside_triangle({ (float)x, (float)y }, a, b, c, alpha, beta, gamma))
            {
                ...
                {
                    Vec3 interpolated = alpha * c0 + beta * c1 + gamma * c2;
                    Color color = {
                        (unsigned char)(std::min(interpolated.x * 255.0f, 255.0f)),
                        (unsigned char)(std::min(interpolated.y * 255.0f, 255.0f)),
                        (unsigned char)(std::min(interpolated.z * 255.0f, 255.0f))
                    };
...
void render_scene()
{
    ...
    for (int i = 0; i < gNumTriangles; ++i)
    {
        ...
        Vec3 v0 = gVertexBuffer[k0];
        Vec3 v1 = gVertexBuffer[k1];
        Vec3 v2 = gVertexBuffer[k2];

        Vec3 n0 = normalize(v0);
        Vec3 n1 = normalize(v1);
        Vec3 n2 = normalize(v2);

        Vec3 c0 = compute_vertex_color(v0, n0);
        Vec3 c1 = compute_vertex_color(v1, n1);
        Vec3 c2 = compute_vertex_color(v2, n2);

        draw_triangle(v0, v1, v2, c0, c1, c2);
    }
}
